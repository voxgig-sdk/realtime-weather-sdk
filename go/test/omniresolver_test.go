...
import (
	...
	omni "github.com/voxgig-sdk/realtime-weather-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RealtimeWeatherError); is {
				out := map[string]any{
					"name":    "RealtimeWeatherError",
...
func omniCtx(arg any, client *sdk.RealtimeWeatherSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RealtimeWeatherSDK); is {
			client = live
