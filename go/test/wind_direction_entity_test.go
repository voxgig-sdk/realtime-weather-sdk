...
func TestWindDirectionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.WindDirection(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.WindDirection(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		windDirectionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.wind_direction")))
...
func wind_directionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REALTIME_WEATHER_TEST_WIND_DIRECTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REALTIME_WEATHER_TEST_WIND_DIRECTION_ENTID"])
