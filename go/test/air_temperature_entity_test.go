...
func TestAirTemperatureEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.AirTemperature(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.AirTemperature(nil).Stream("list", nil, nil) {
...
func air_temperatureBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("REALTIME_WEATHER_TEST_AIR_TEMPERATURE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["REALTIME_WEATHER_TEST_AIR_TEMPERATURE_ENTID"])
