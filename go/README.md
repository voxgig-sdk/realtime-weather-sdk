...
## Install
```bash
go get github.com/voxgig-sdk/realtime-weather-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/realtime-weather-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/realtime-weather-sdk/go=../realtime-weather-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *RealtimeWeatherSDK
```
...
| `WindDirection` | `(data map[string]any) RealtimeWeatherEntity` | Create a WindDirection entity instance. |
| `WindSpeed` | `(data map[string]any) RealtimeWeatherEntity` | Create a WindSpeed entity instance. |

### Entity interface (RealtimeWeatherEntity)

All entities implement the `RealtimeWeatherEntity` interface.
...
Create an instance: `airTemperature := client.AirTemperature(nil)`
...
fmt.Println(airTemperatures) // the array of records
```
...
Create an instance: `rainfall := client.Rainfall(nil)`
...
Create an instance: `relativeHumidity := client.RelativeHumidity(nil)`
...
```go
relativeHumiditys, err := client.RelativeHumidity(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(relativeHumiditys) // the array of records
```


### WindDirection

Create an instance: `windDirection := client.WindDirection(nil)`
...
fmt.Println(windDirections) // the array of records
```


### WindSpeed

Create an instance: `windSpeed := client.WindSpeed(nil)`
...
```go
windSpeeds, err := client.WindSpeed(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(windSpeeds) // the array of records
```
...
```
github.com/voxgig-sdk/realtime-weather-sdk/go/
├── realtime-weather.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/realtime-weather-sdk/go`) re-exports everything needed
