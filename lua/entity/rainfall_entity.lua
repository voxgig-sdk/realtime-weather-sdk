...
local RainfallEntity = {}
RainfallEntity.__index = RainfallEntity


function RainfallEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RainfallEntity)
  self._name = "rainfall"
  self._client = client
...
function RainfallEntity:get_name()
  return self._name
end


function RainfallEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RainfallEntity.new(self._client, opts)
end
...
function RainfallEntity:mark_deleted()
  self._deleted = true
end


function RainfallEntity:deleted()
  return true == self._deleted
end


function RainfallEntity:data_set(args)
  if args ~= nil then
...
function RainfallEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RainfallEntity:match_set(args)
  if args ~= nil then
...
function RainfallEntity:match_get()
...
function RainfallEntity:stream(action, args, callopts)
...
function RainfallEntity:_run_op(ctx, post_done)
  local utility = self._utility
