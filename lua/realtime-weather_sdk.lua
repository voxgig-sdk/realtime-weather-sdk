...
require("realtime-weather_types")
...
local RealtimeWeatherSDK = {}
RealtimeWeatherSDK.__index = RealtimeWeatherSDK
...
RealtimeWeatherSDK._make_feature = _make_feature


function RealtimeWeatherSDK.new(options)
  local self = setmetatable({}, RealtimeWeatherSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RealtimeWeatherSDK:options_map()
...
function RealtimeWeatherSDK:get_utility()
  return Utility.copy(self._utility)
end


function RealtimeWeatherSDK:get_root_ctx()
  return self._rootctx
end


function RealtimeWeatherSDK:prepare(fetchargs)
...
function RealtimeWeatherSDK:direct(fetchargs)
...
function RealtimeWeatherSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function RealtimeWeatherSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "RealtimeWeatherSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function RealtimeWeatherSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RealtimeWeatherSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RealtimeWeatherSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RealtimeWeatherSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RealtimeWeatherSDK.new(sdkopts)
  sdk.mode = "test"
