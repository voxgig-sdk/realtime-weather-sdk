# RealtimeWeather Lua SDK



The Lua SDK for the RealtimeWeather API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:AirTemperature()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```
lua/
├── realtime-weather_sdk.lua    -- Main SDK module
...
The main module (`realtime-weather_sdk`) exports the SDK constructor
