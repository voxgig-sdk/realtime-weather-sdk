...
The Lua SDK for the RealtimeWeather API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AirTemperature(nil):list(nil, nil)
...
local result, err = client:RealtimeWeather(nil):load(
  { id = "test01" }, nil
)
