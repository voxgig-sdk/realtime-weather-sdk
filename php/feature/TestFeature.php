...
// RealtimeWeather SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RealtimeWeatherTestFeature extends RealtimeWeatherBaseFeature
{
    ...
    public function init(RealtimeWeatherContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RealtimeWeatherContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RealtimeWeatherParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RealtimeWeatherContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RealtimeWeatherContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RealtimeWeatherParam::call($ctx, $k);
