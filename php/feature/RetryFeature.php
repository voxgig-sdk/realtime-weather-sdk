...
// RealtimeWeather SDK retry feature
...
class RealtimeWeatherRetryFeature extends RealtimeWeatherBaseFeature
{
    ...
    public function init(RealtimeWeatherContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RealtimeWeatherContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RealtimeWeatherContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RealtimeWeatherContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
