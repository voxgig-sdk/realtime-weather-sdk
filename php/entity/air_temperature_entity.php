...
// RealtimeWeather SDK AirTemperature entity
...
class AirTemperatureEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AirTemperatureEntity($this->_client, $opts);
    }

    /**
     * @param AirTemperature|array $args AirTemperature data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RealtimeWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return AirTemperature|array The current AirTemperature data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of AirTemperature fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RealtimeWeatherHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of AirTemperature fields).
     */
    ...
    /**
     * List AirTemperature items matching the given filter.
     *
     * @param AirTemperatureListMatch|array|null $reqmatch Match filter (any subset
     *   of AirTemperature fields) as an assoc-array; AirTemperatureListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return AirTemperature[]|array A list of AirTemperature items as assoc-arrays at
