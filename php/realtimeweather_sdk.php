...
class RealtimeWeatherSDK
{
        ...
        if ($feature_opts) {
            ...
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = RealtimeWeatherHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!RealtimeWeatherFeatures::has_feature($fname)) {
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new RealtimeWeatherError($op . "_allow",
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new RealtimeWeatherError("graphql_error",
                "RealtimeWeatherSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
