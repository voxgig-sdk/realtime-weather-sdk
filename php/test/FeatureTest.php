...
// RealtimeWeather SDK feature test
//
...
class FtCtrl extends RealtimeWeatherControl
{
...
class FtClient
{
    ...
    private RealtimeWeatherUtility $utility;

    public function __construct(RealtimeWeatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): RealtimeWeatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RealtimeWeatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RealtimeWeatherContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RealtimeWeatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RealtimeWeatherConfig::make_feature($name);
    ...
    private static function buildUrl(RealtimeWeatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RealtimeWeatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RealtimeWeatherResponse($fetched);
        }
    ...
    private function fail(RealtimeWeatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RealtimeWeatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
