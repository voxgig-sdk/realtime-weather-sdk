...
class WindSpeedDirectTest extends TestCase
{
    public function test_direct_list_wind_speed(): void
    {
        $setup = wind_speed_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-wind_speed", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["collection01"] as $_liveKey) {
...
function wind_speed_direct_setup($mockres)
{
