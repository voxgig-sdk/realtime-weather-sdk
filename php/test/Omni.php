...
//    only - a typed RealtimeWeatherContext there would make every ctx
...
final class RealtimeWeatherOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RealtimeWeatherContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new RealtimeWeatherSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new RealtimeWeatherResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new RealtimeWeatherError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new RealtimeWeatherResponse($clean['response']);
    ...
    public static function err_from_map($m): ?RealtimeWeatherError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RealtimeWeatherError(is_string($code) ? $code : '', $msg);
    }
