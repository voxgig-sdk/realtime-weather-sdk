...
module RealtimeWeatherOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if RealtimeWeatherOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = RealtimeWeatherOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| RealtimeWeatherOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || RealtimeWeatherOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = RealtimeWeatherSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = RealtimeWeatherError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { RealtimeWeatherOmni.errify(err) }
