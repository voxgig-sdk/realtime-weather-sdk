...
// RealtimeWeather SDK pipeline test
//
...
{
    private static function utility(): RealtimeWeatherUtility
    {
        return new RealtimeWeatherUtility();
    }
    ...
    private static function ctx(array $over = []): RealtimeWeatherContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RealtimeWeatherContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RealtimeWeatherOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RealtimeWeatherError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RealtimeWeatherOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RealtimeWeatherMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RealtimeWeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RealtimeWeatherMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RealtimeWeatherOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RealtimeWeatherMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealtimeWeatherSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RealtimeWeatherMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RealtimeWeatherResponse([]),
            'result' => new RealtimeWeatherResult([]),
        ]);
        [, $err] = RealtimeWeatherMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec([]),
            'response' => null,
            'result' => new RealtimeWeatherResult([]),
        ]);
        [, $err] = RealtimeWeatherMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec(['step' => 's']),
            'response' => new RealtimeWeatherResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RealtimeWeatherResult([]),
        ]);
        [, $err] = RealtimeWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec(['step' => 's']),
            'response' => new RealtimeWeatherResponse(self::resp(200, ['v' => 1])),
            'result' => new RealtimeWeatherResult([]),
        ]);
        [, $err] = RealtimeWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec(['step' => 's']),
            'response' => new RealtimeWeatherResponse(self::resp(200, ['v' => 2])),
            'result' => new RealtimeWeatherResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RealtimeWeatherMakeResponse::call($ctx);
    ...
    {
        $preset = new RealtimeWeatherResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec([]),
            'response' => new RealtimeWeatherResponse([]),
            'result' => new RealtimeWeatherResult([]),
        ...
        [$got, $err] = RealtimeWeatherMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RealtimeWeatherResult([])]);
        [, $err] = RealtimeWeatherMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RealtimeWeatherSpec([]), 'result' => null]);
        [, $err] = RealtimeWeatherMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealtimeWeatherSpec(['step' => 's']),
            'result' => new RealtimeWeatherResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RealtimeWeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealtimeWeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RealtimeWeatherSpec(['step' => 's']),
            'result' => new RealtimeWeatherResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RealtimeWeatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RealtimeWeatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RealtimeWeatherResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RealtimeWeatherSpec([]),
            'result' => new RealtimeWeatherResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RealtimeWeatherMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealtimeWeatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RealtimeWeatherError('boom', 'boom');
        $utility->fetcher = function (RealtimeWeatherContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealtimeWeatherSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RealtimeWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RealtimeWeatherContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RealtimeWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RealtimeWeatherResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RealtimeWeatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RealtimeWeatherContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RealtimeWeatherSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RealtimeWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RealtimeWeatherResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RealtimeWeatherSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RealtimeWeatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RealtimeWeatherMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RealtimeWeatherMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RealtimeWeatherResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RealtimeWeatherDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RealtimeWeatherResult(['ok' => false])]);
        $this->expectException(RealtimeWeatherError::class);
        RealtimeWeatherDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RealtimeWeatherResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RealtimeWeatherMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RealtimeWeatherMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RealtimeWeatherBaseFeature();
        $b = new RealtimeWeatherBaseFeature();
        RealtimeWeatherFeatureAdd::call($ctx, $a);
        RealtimeWeatherFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RealtimeWeatherBaseFeature
    {
        $f = new RealtimeWeatherBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RealtimeWeatherFeatureAdd::call($ctx, self::named_feature('a'));
        RealtimeWeatherFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        RealtimeWeatherFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        RealtimeWeatherFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        RealtimeWeatherFeatureAdd::call($ctx, $replace);
        ...
        RealtimeWeatherFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RealtimeWeatherContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RealtimeWeatherSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RealtimeWeatherPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RealtimeWeatherPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RealtimeWeatherResponse(['headers' => null]),
            'result' => new RealtimeWeatherResult([]),
        ]);
        RealtimeWeatherResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RealtimeWeatherResultBody::call($ctx);
