...
//     (\`new RealtimeWeatherSDK(...)\` / \`RealtimeWeatherSDK.test(...)\` become
//     \`RealtimeWeatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RealtimeWeatherSDK'
...
const TEST_SEED = {"entity":{"air_temperature":{"test01":{"id":"test01"}},"collection":{"test01":{"id":"test01"}},"rainfall":{"test01":{"id":"test01"}},"relative_humidity":{"test01":{"id":"test01"}},"wind_direction":{"test01":{"id":"test01"}},"wind_speed":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RealtimeWeatherSDK(...)\` and any
// \`RealtimeWeatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RealtimeWeatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RealtimeWeatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
