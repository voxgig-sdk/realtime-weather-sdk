# realtime-weather-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RealtimeWeather SDK as
two agent tools — `realtime-weather_list` and `realtime-weather_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/realtime-weather-mcp)
make build

# 2. Provide credentials via the environment
export REALTIME_WEATHER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user realtime-weather \
  -- /absolute/path/to/realtime-weather-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./realtime-weather-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// realtime-weather_list: first page of records
{ "entity": "air_temperature" }
{ "entity": "air_temperature", "query": { } }

// realtime-weather_load: one record by id
{ "entity": "air_temperature", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/realtime-weather-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user realtime-weather \
     -- "$PWD"/dist/*/realtime-weather-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `realtime-weather_list` and `realtime-weather_load` tools now appear
   in new sessions. Ask the agent to *"list air_temperature using realtime-weather"*
   and it calls `realtime-weather_list` with `{"entity":"air_temperature"}`.
...
```sh
export REALTIME_WEATHER_APIKEY=sk_live_xxx            # API key
export REALTIME_WEATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `realtime-weather_list` tool
...
### Call the `realtime-weather_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `realtime-weather_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `realtime-weather_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `REALTIME_WEATHER_APIKEY` | API key sent with every request. |
| `REALTIME_WEATHER_BASE` | Optional override of the API base URL. |
...
```sh
./realtime-weather-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"realtime-weather_load","arguments":{"entity":"air_temperature","query":{"id":1}}}}'
```
