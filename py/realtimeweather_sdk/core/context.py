...
from realtimeweather_sdk.core.helpers import get_ctx_prop, to_map
...
class RealtimeWeatherContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        # Ctrl
        self.ctrl = RealtimeWeatherControl()
        ...
        if isinstance(sp, RealtimeWeatherSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, RealtimeWeatherResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, RealtimeWeatherResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def resolve_op(self, opname):
        ...
        if opname == "":
            return RealtimeWeatherOperation({})
    ...
    def make_error(self, code, msg):
        return RealtimeWeatherError(code, msg, self)
