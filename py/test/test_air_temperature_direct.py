...
class TestAirTemperatureDirect:

    def test_should_direct_list_air_temperature(self):
        setup = _air_temperature_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-air_temperature", "live" if setup["live"] else "unit")
        if _skip:
...
def _air_temperature_direct_setup(mockres):
