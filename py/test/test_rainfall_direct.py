...
class TestRainfallDirect:

    def test_should_direct_list_rainfall(self):
        setup = _rainfall_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-rainfall", "live" if setup["live"] else "unit")
        if _skip:
...
def _rainfall_direct_setup(mockres):
