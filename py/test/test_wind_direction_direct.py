...
class TestWindDirectionDirect:

    def test_should_direct_list_wind_direction(self):
        setup = _wind_direction_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-wind_direction", "live" if setup["live"] else "unit")
        if _skip:
...
def _wind_direction_direct_setup(mockres):
