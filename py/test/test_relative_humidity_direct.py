...
class TestRelativeHumidityDirect:

    def test_should_direct_list_relative_humidity(self):
        setup = _relative_humidity_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-relative_humidity", "live" if setup["live"] else "unit")
        if _skip:
...
def _relative_humidity_direct_setup(mockres):
