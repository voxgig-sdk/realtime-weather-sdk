...
class TestWindDirectionEntity:

    def test_should_create_instance(self):
        testsdk = RealtimeWeatherSDK.test(None, None)
        ent = testsdk.WindDirection(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.WindDirection(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.WindDirection(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _wind_direction_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "wind_direction." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REALTIME_WEATHER_TEST_WIND_DIRECTION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        wind_direction_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.wind_direction")))
        wind_direction_ref01_data = None
        if len(wind_direction_ref01_data_raw) > 0:
            wind_direction_ref01_data = helpers.to_map(wind_direction_ref01_data_raw[0][1])

        # LIST
        wind_direction_ref01_ent = client.WindDirection(None)
        wind_direction_ref01_match = {
            "collection_id": setup["idmap"]["collection01"],
        }

        wind_direction_ref01_list_result = wind_direction_ref01_ent.list(wind_direction_ref01_match, None)
        assert isinstance(wind_direction_ref01_list_result, list)



def _wind_direction_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/wind_direction/WindDirectionTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("REALTIME_WEATHER_TEST_WIND_DIRECTION_ENTID"))
