...
class TestWindSpeedDirect:

    def test_should_direct_list_wind_speed(self):
        setup = _wind_speed_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-wind_speed", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["collection01"]:
...
def _wind_speed_direct_setup(mockres):
