# RealtimeWeather SDK test runner
...
class RealtimeWeatherTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RealtimeWeatherTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RealtimeWeatherTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RealtimeWeatherTestRunner.getenv("REALTIME_WEATHER_TEST_LIVE")
        override = RealtimeWeatherTestRunner.getenv("REALTIME_WEATHER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RealtimeWeatherTestRunner.getenv(key)
        ...
        explain = RealtimeWeatherTestRunner.getenv("REALTIME_WEATHER_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RealtimeWeatherTestRunner._test_control is not None:
            return RealtimeWeatherTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RealtimeWeatherTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RealtimeWeatherTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RealtimeWeatherTestRunner.load_test_control()
...
def load_env_local():
    RealtimeWeatherTestRunner.load_env_local()


def env_override(m):
    return RealtimeWeatherTestRunner.env_override(m)


def entity_data(v):
    return RealtimeWeatherTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RealtimeWeatherTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RealtimeWeatherTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RealtimeWeatherTestRunner.load_test_control()


def live_client_options():
    return RealtimeWeatherTestRunner.live_client_options()


def live_delay_ms():
    return RealtimeWeatherTestRunner.live_delay_ms()
