...
class TestRelativeHumidityEntity:

    def test_should_create_instance(self):
        testsdk = RealtimeWeatherSDK.test(None, None)
        ent = testsdk.RelativeHumidity(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.RelativeHumidity(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.RelativeHumidity(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _relative_humidity_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "relative_humidity." + _op, "live" if _live else "unit")
            if _skip:
        ...
        relative_humidity_ref01_data = None
        if len(relative_humidity_ref01_data_raw) > 0:
            relative_humidity_ref01_data = helpers.to_map(relative_humidity_ref01_data_raw[0][1])

        # LIST
        relative_humidity_ref01_ent = client.RelativeHumidity(None)
        relative_humidity_ref01_match = {
            "collection_id": setup["idmap"]["collection01"],
        }

        relative_humidity_ref01_list_result = relative_humidity_ref01_ent.list(relative_humidity_ref01_match, None)
        assert isinstance(relative_humidity_ref01_list_result, list)



def _relative_humidity_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/relative_humidity/RelativeHumidityTestData.json")
