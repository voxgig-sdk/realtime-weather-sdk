...
class TestRainfallEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Rainfall(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Rainfall(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set REALTIME_WEATHER_TEST_RAINFALL_ENTID JSON to run live")
        client = setup["client"]
...
def _rainfall_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("REALTIME_WEATHER_TEST_RAINFALL_ENTID"))
