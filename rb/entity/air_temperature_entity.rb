...
class AirTemperatureEntity
  ...
  def make
    opts = @_entopts.dup
    AirTemperatureEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RealtimeWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AirTemperature, Hash] the current AirTemperature data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RealtimeWeatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AirTemperature fields)
  def match_get
  ...
  #
  # @param reqmatch [AirTemperatureListMatch, Hash, nil] match filter (any subset of
  #   AirTemperature fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<AirTemperature>, Array] the matching AirTemperature items; raises RealtimeWeatherError on failure
