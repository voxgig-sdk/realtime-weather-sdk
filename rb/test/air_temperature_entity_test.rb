...
class AirTemperatureEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.AirTemperature(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.AirTemperature(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = air_temperature_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "air_temperature." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    air_temperature_ref01_data = nil
    if air_temperature_ref01_data_raw.length > 0
      air_temperature_ref01_data = Helpers.to_map(air_temperature_ref01_data_raw[0][1])
    end

    # LIST
    air_temperature_ref01_ent = client.AirTemperature(nil)
    air_temperature_ref01_match = {
      "collection_id" => setup[:idmap]["collection01"],
    }

    air_temperature_ref01_list_result = air_temperature_ref01_ent.list(air_temperature_ref01_match, nil)
    assert air_temperature_ref01_list_result.is_a?(Array)

  end
end

def air_temperature_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "air_temperature", "AirTemperatureTestData.json")
