...
class AirTemperatureDirectTest < Minitest::Test
  def test_direct_list_air_temperature
    setup = air_temperature_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-air_temperature", setup[:live] ? "live" : "unit")
    if _should_skip
...
def air_temperature_direct_setup(mockres)
