...
class WindDirectionDirectTest < Minitest::Test
  def test_direct_list_wind_direction
    setup = wind_direction_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-wind_direction", setup[:live] ? "live" : "unit")
    if _should_skip
...
def wind_direction_direct_setup(mockres)
