...
class RelativeHumidityEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.RelativeHumidity(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.RelativeHumidity(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REALTIME_WEATHER_TEST_RELATIVE_HUMIDITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    relative_humidity_ref01_list_result = relative_humidity_ref01_ent.list(relative_humidity_ref01_match, nil)
...
def relative_humidity_basic_setup(extra)
  ...
  entid_env_raw = ENV["REALTIME_WEATHER_TEST_RELATIVE_HUMIDITY_ENTID"]
