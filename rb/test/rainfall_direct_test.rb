...
class RainfallDirectTest < Minitest::Test
  def test_direct_list_rainfall
    setup = rainfall_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-rainfall", setup[:live] ? "live" : "unit")
    if _should_skip
...
def rainfall_direct_setup(mockres)
