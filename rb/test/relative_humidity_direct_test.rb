...
class RelativeHumidityDirectTest < Minitest::Test
  def test_direct_list_relative_humidity
    setup = relative_humidity_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-relative_humidity", setup[:live] ? "live" : "unit")
    if _should_skip
...
def relative_humidity_direct_setup(mockres)
