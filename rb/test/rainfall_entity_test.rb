...
class RainfallEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Rainfall(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Rainfall(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = rainfall_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "rainfall." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set REALTIME_WEATHER_TEST_RAINFALL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    rainfall_ref01_data = nil
    if rainfall_ref01_data_raw.length > 0
      rainfall_ref01_data = Helpers.to_map(rainfall_ref01_data_raw[0][1])
    end

    # LIST
    rainfall_ref01_ent = client.Rainfall(nil)
...
def rainfall_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "rainfall", "RainfallTestData.json")
  ...
  entid_env_raw = ENV["REALTIME_WEATHER_TEST_RAINFALL_ENTID"]
