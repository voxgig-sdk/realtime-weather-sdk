...
class WindSpeedDirectTest < Minitest::Test
  def test_direct_list_wind_speed
    setup = wind_speed_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-wind_speed", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["collection01"].each do |_live_key|
...
def wind_speed_direct_setup(mockres)
