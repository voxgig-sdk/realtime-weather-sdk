...
require_relative 'RealtimeWeather_types'
...
class RealtimeWeatherSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RealtimeWeatherHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RealtimeWeatherHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RealtimeWeatherFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RealtimeWeatherUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RealtimeWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RealtimeWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RealtimeWeatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    rescue RealtimeWeatherError => err
    ...
    if fetched.is_a?(Hash)
      status = RealtimeWeatherHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RealtimeWeatherError.new(
        "graphql_error", "RealtimeWeatherSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RealtimeWeatherSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
