...
module RealtimeWeatherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RealtimeWeatherResponse.new({})
    ...
    if fetch_err
      ...
      response = RealtimeWeatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RealtimeWeatherResponse.new(fetched)
    else
