// Command RealtimeWeather-cli is a CLI + REPL for the RealtimeWeather SDK,
...
const prompt = "realtime-weather"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "air_temperature collection rainfall relative_humidity wind_direction wind_speed"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      REALTIME_WEATHER_APIKEY (api key), REALTIME_WEATHER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: REALTIME_WEATHER_APIKEY carries the API key and
	// REALTIME_WEATHER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("REALTIME_WEATHER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("REALTIME_WEATHER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRealtimeWeatherSDK(opts)
