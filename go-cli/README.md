...
boru-driven command-line client **and** interactive REPL for the RealtimeWeather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/realtime-weather-cli)
make build

# 2. See usage (words, entities, env vars)
./realtime-weather-cli --help
...
./realtime-weather-cli list collection

# 5. Override the API base URL for a single call
REALTIME_WEATHER_BASE=https://api.example.com ./realtime-weather-cli list air_temperature

# 6. No arguments -> interactive REPL
./realtime-weather-cli
realtime-weather> list air_temperature
realtime-weather> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/realtime-weather-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/realtime-weather-cli list air_temperature
   ```
...
Run with no arguments to open a REPL (prompt `realtime-weather>`). Each line is
evaluated as its own boru expression:

```text
$ ./realtime-weather-cli
realtime-weather> list air_temperature
realtime-weather> /help
realtime-weather> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `air_temperature`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/realtime-weather-cli`. |
...
(`./realtime-weather-cli <expr>`) and interactively (the REPL), and expressions compose the
